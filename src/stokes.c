 ...
 * $Id: stokes.c,v 2.4 2006/10/05 00:25:12 kichiki Exp $
 *
		 ...
		 double zeta, double cutlim)
{
  ...
  sys->self_a = 1.0  - sys->zaspi * (6.0 - 40.0 / 3.0 * sys->za2);
  sys->self_c = 0.75 - sys->zaspi * sys->za2 * 10.0;
  sys->self_m = 0.9  - sys->zaspi * sys->za2 * (12.0 - 30.24 * sys->za2);
  ...
  dk = 2.0 * M_PI / lmax;
  /* in real space */
  rmax = 10.0 * sqrt (- log (cutlim)) / sys->zeta;
  for (;
       (ewald_real (rmax * red_factor, sys->zeta)
	+ 6.0 * ewald_real (rmax * red_factor + dr, sys->zeta))
	 < cutlim;
       rmax *= red_factor);
  sys->rmax = rmax;
  ...
  /* in reciprocal space */
  kmax = 10.0 * sqrt (- log (cutlim)) * 2.0 * sys->zeta;
  for (;
       (ewald_recip (kmax * red_factor, sys->zeta)
	+ 6.0 * ewald_recip (kmax * red_factor + dk, sys->zeta))
	 < cutlim;
       kmax *= red_factor);
  sys->kmax = kmax;
