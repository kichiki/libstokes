 ...
 * $Id: ewald-3f-matrix.c,v 2.10 2006/10/19 18:26:00 ichiki Exp $
 *
			   ...
			   double *f)
{
  ...
  // f := M^-1.u
  dot_prod_matrix (mat, n3, n3, u0, f);
  free (u0);
			       ...
			       double *f)
{
  ...
  double *u0;
  u0  = (double *) malloc (sizeof (double) * n3);
  mob = (double *) malloc (sizeof (double) * n3 * n3);
  ...
  if (u0 == NULL ||
    ...
    {
      fprintf (stderr, "libstokes: allocation error"
	       " at solve_res_lub_ewald_3f_matrix()\n");
      exit (1);
    }
  ...
  // x := (M^-1 + L).(UO)
  dot_prod_matrix (lub, n3, n3, u0, f);
  free (u0);
			       ...
			       double *u)
{
  ...
  if (mat == NULL ||
    ...
    {
      fprintf (stderr, "libstokes: allocation error"
	       " at solve_mob_lub_ewald_3f_matrix()\n");
      exit (1);
    }
			   ...
			   double *u, double *ff)
{
  ...
  if (np == nm)
    {
      solve_mob_ewald_3f_matrix (sys, f, u);
      return;
    }
			       ...
			       double *u, double *ff)
{
  ...
  if (uf0 == NULL ||
    ...
    {
      fprintf (stderr, "libstokes: allocation error"
	       " at solve_mix_lub_ewald_3f_matrix()\n");
      exit (1);
    }
