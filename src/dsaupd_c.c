...
*/
void dseupd_
(int *rvec, char *howmny, int *select, double *d, double *z, 
 int *ldz, double *sigma, char *bmat,
 int *n, char *which, int *nev, double *tol, double *resid, int *ncv, 
 double *v, int *ldv, int *iparam, 
 int *ipntr, double *workd, double *workl, int *lworkl, int *info );


static void
dsaupd_info (FILE *out, int info)
{
  if (info == 0)
    {
      fprintf (out, "[dsaupd]: Normal exit.\n");
    }
  else if (info == 1)
    {
      fprintf (out, "[dsaupd]: Maximum number of iterations taken. "
    ...
    {
      fprintf (out, "[dsaupd]: N must be positive.\n");
    }
  else if (info == -2)
    {
      fprintf (out, "[dsaupd]: NEV must be positive.\n");
    }
  else if (info == -3)
    {
      fprintf (out, "[dsaupd]: NCV must be greater than NEV and less than or equal to N.\n");
    }
  else if (info == -4)
    {
      fprintf (out, "[dsaupd]: "
    ...
    {
      fprintf (out, "[dsaupd]: WHICH must be one of LM, SM, LA, SA or BE.\n");
    }
  else if (info == -6)
    {
      fprintf (out, "[dsaupd]: BMAT must be one of I or G.\n");
    }
  else if (info == -7)
    {
      fprintf (out, "[dsaupd]: Length of private work array WORKL is not sufficient.\n");
    }
  else if (info == -8)
    {
      fprintf (out, "[dsaupd]: Error return from trid. eigenvalue calculation; "
	       "Informatinal error from LAPACK routine dsteqr.\n");
    }
  else if (info == -9)
    {
      fprintf (out, "[dsaupd]: Starting vector is zero.\n");
    }
  else if (info == -10)
    {
      fprintf (out, "[dsaupd]: IPARAM(7) must be 1,2,3,4,5.\n");
    }
  else if (info == -11)
    {
      fprintf (out, "[dsaupd]: IPARAM(7) = 1 and BMAT = G are incompatable.\n");
    }
  else if (info == -12)
    {
      fprintf (out, "[dsaupd]: IPARAM(1) must be equal to 0 or 1.\n");
    }
  else if (info == -13)
    {
      fprintf (out, "[dsaupd]: NEV and WHICH = BE are incompatable.\n");
    }
  else if (info == -9999)
    {
      fprintf (out, "[dsaupd]: Could not build an Arnoldi factorization. "
    ...
    {
      fprintf (out, "[dsaupd]: unknown error: info = %d\n", info);
    }
}

static void
dseupd_info (FILE *out, int info)
{
  if (info == 0)
    {
      fprintf (out, "[dseupd]: Normal exit.\n");
    }
  else if (info == -1)
    {
      fprintf (out, "[dseupd]: N must be positive.\n");
    }
  else if (info == -2)
    {
      fprintf (out, "[dseupd]: NEV must be positive.\n");
    }
  else if (info == -3)
    {
      fprintf (out, "[dseupd]: NCV must be greater than NEV and less than or equal to N.\n");
    }
  else if (info == -5)
    {
      fprintf (out, "[dseupd]: WHICH must be one of LM, SM, LA, SA or BE.\n");
    }
  else if (info == -6)
    {
      fprintf (out, "[dseupd]: BMAT must be one of I or G.\n");
    }
  else if (info == -7)
    {
      fprintf (out, "[dseupd]: Length of private work WORKL array is not sufficient.\n");
    }
  else if (info == -8)
    {
      fprintf (out, "[dseupd]: "
    ...
    {
      fprintf (out, "[dseupd]: Starting vector is zero.\n");
    }
  else if (info == -10)
    {
      fprintf (out, "[dseupd]: IPARAM(7) must be 1,2,3,4,5.\n");
    }
  else if (info == -11)
    {
      fprintf (out, "[dseupd]: IPARAM(7) = 1 and BMAT = G are incompatible.\n");
    }
  else if (info == -12)
    {
      fprintf (out, "[dseupd]: NEV and WHICH = BE are incompatible.\n");
    }
  else if (info == -14)
    {
      fprintf (out, "[dseupd]: "
    ...
    {
      fprintf (out, "[dseupd]: HOWMNY = S not yet implemented.\n");
    }
  else 
    {
      fprintf (out, "[dseupd]: unknown error: info = %d\n", info);
    }
}
